...
while (($remoteImage == "not-found.png") && ($nb_tries<10)) {
    ...
    /*
    $ca = curl_init();
    curl_setopt($ca, CURLOPT_URL, "http://api.themoviedb.org/3/configuration?api_key=".TMDB_API_KEY);
    ...
    */
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://api.themoviedb.org/3/search/movie?query=".urlencode($_GET['t'])."&year=".urlencode($_GET['y'])."&api_key=".TMDB_API_KEY);
